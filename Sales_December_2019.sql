...
WHERE TABLE_NAME = 'Sales_December_2019' 
...
SELECT COUNT(*) FROM Sales_December_2019
...
SELECT DISTINCT Product FROM Sales_December_2019
...
SELECT Product, MIN(Price_Each) Max_Price, 
	MAX(Price_Each) AS Min_Price FROM Sales_December_2019
	group by Product
...
--- change the data type of order date, and any abnormal values
select Order_Date, CAST(Order_Date AS smalldatetime) as 'Formatted Date',MONTH(CAST(Order_Date AS smalldatetime)) as 'MONTH'
FROM Sales_December_2019

--- update the Order date where there are wrong wrong entries
UPDATE Sales_December_2019
SET Order_Date = REPLACE(REPLACE(Order_Date,'-','/'),'2012','12')
where Order_Date Like '2012%'

--- update Order date with correct data type
UPDATE Sales_December_2019
SET Order_Date = CAST(Order_Date AS smalldatetime)
FROM Sales_December_2019
...
UPDATE Sales_December_2019
SET Month_Number = MONTH(Order_Date) FROM Sales_December_2019

--- explore to know the months
SELECT Month_Number AS MONTH FROM Sales_December_2019
GROUP BY Month_Number
/* Found  data to contain for the January month too, so these records needs to be removed to perform analytics for entire quarter from October to December*/
...
SELECT DISTINCT State_Name FROM Sales_December_2019

----  check if the pincode IS neat and clean
SELECT Pincode FROM Sales_December_2019
